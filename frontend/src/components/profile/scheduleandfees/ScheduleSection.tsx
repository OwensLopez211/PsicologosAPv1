    ...
    if (activeDays.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-6 text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">Haz clic en "Editar" para configurar tus horarios de atención</p>
        </div>
      );
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {activeDays.map(day => {
          ...
          return (
            <motion.div 
              key={day.id} 
              className={`rounded-lg bg-gradient-to-br ${dayInfo?.color} p-4 border border-gray-100`}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3 }}
            >
              <div className="font-medium text-gray-700 text-lg mb-2 border-b border-gray-200 pb-1">
                {day.label}
              </div>
              <div className="space-y-2">
                {localSchedule[day.id].timeBlocks.map((block, index) => (
                  <div 
                    key={index} 
                    className="flex items-center gap-2 bg-white/70 backdrop-blur-sm p-2 rounded-lg shadow-sm"
                  >
                    <ClockIcon className="h-4 w-4 text-[#2A6877]" />
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Define cuándo estás disponible para atender pacientes</p>
          </div>
        </div>
      ...
      <AnimatePresence>
        {!isEditing && showScheduleSummary && (
          <motion.div 
            ...
            className="bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-4 mt-4"
          >
            <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center">
              <ClockIcon className="w-5 h-5 mr-2 text-[#2A6877]" />
              Tu horario actual
            </h3>
            {getScheduleSummary()}
          </motion.div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {daysOfWeek.map(({ id, label, color }) => (
                <motion.div 
                  key={id} 
                  className={`rounded-lg p-4 transition-all bg-white shadow-sm border ${
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <label className="flex items-center cursor-pointer">
                      ...
                      />
                      <span className={`ml-2 font-semibold ${localSchedule[id].enabled ? 'text-gray-800' : 'text-gray-500'}`}>
                        {label}
                      </span>
                    </label>
                    {localSchedule[id].enabled && (
                      <motion.button
                        ...
                        className="px-2 py-1 bg-white/80 text-sm text-[#2A6877] hover:text-white hover:bg-[#2A6877] rounded-full border border-[#2A6877]/30 flex items-center shadow-sm"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
                    ...
                    {localSchedule[id].enabled && (
                      ...
                      >
                        {localSchedule[id].timeBlocks.map((block, blockIndex) => (
                          <motion.div 
                            key={blockIndex} 
                            className="flex items-center gap-2 bg-white/90 p-3 rounded-lg border border-[#2A6877]/20 shadow-sm"
                            initial={{ opacity: 0, y: -10 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: blockIndex * 0.1 }}
                            whileHover={{ y: -1 }}
                          >
                            <ClockIcon className="h-4 w-4 text-[#2A6877] flex-shrink-0" />
                            <input
                              type="time"
                            ...
                            {localSchedule[id].timeBlocks.length > 1 && (
                              <motion.button
                                ...
                                className="bg-red-50 text-red-500 hover:text-red-700 hover:bg-red-100 p-1.5 rounded-full"
                        ...
                        {localSchedule[id].timeBlocks.length === 0 && (
                          <motion.div 
                            className="flex items-center justify-center p-4 bg-white/50 rounded-lg border border-dashed border-gray-300"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                          >
                            <button
                            ...
                            >
                              <PlusIcon className="h-6 w-6 mb-1 text-[#2A6877]" />
                              Agregar un horario
                            </button>
                          </motion.div>
                        )}
