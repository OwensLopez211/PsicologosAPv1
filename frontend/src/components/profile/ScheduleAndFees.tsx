...
const ScheduleAndFees = ({ profile, onSave, isLoading, onLoadingChange }: ScheduleAndFeesProps) => {
  ...
  // Modify the existing useEffect to handle profile updates
  useEffect(() => {
    console.log("Profile data received:", profile);
    
    if (profile?.schedule_config) {
      try {
        // Process the schedule data from the profile
        ...
        // Don't override existing schedule if there's an error
      }
    }
  }, [profile]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error('Ocurrió un error al actualizar el horario. Por favor, inténtalo de nuevo.');
    } finally {
