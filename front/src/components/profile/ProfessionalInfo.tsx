...
const ProfessionalInfo = ({ profile, onSave, isLoading }: ProfessionalInfoProps) => {
  ...
  return (
    ...
    >
      {/* Encabezado */}
      <motion.div 
        className="bg-white/90 backdrop-blur-sm p-2 sm:p-6 rounded-xl shadow-sm border border-gray-100"
        variants={itemVariants}
      >
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-1.5 sm:gap-3">
            <div className="flex items-center justify-center w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-[#2A6877] to-[#B4E4D3] text-white">
              <motion.span 
            ...
            <div>
              <h2 className="text-sm sm:text-xl font-semibold text-gray-900">Información Profesional</h2>
              <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Define tu perfil profesional para tus pacientes</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            <AnimatePresence>
              {saveSuccess && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className="flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-3 py-0.5 sm:py-1.5 bg-green-50 text-green-600 rounded-full text-xs sm:text-sm"
                >
            ...
            <AnimatePresence>
              {saveError && (
                <motion.div
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: 20 }}
                  className="flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-3 py-0.5 sm:py-1.5 bg-red-50 text-red-600 rounded-full text-xs sm:text-sm"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 w-2.5 sm:h-4 sm:w-4" viewBox="0 0 20 20" fill="currentColor">
            ...
            <AnimatePresence>
              {!isEditing ? (
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 w-2.5 sm:h-4 sm:w-4 mr-0.5 sm:mr-2" viewBox="0 0 20 20" fill="currentColor">
      ...
      <AnimatePresence>
        {isEditing && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 10 }}
            className="bg-amber-50/90 backdrop-blur-sm rounded-xl p-2 sm:p-4 shadow-sm border border-amber-100 z-10 relative mt-2 sm:mt-4"
          >
            <div className="flex items-start gap-1.5 sm:gap-3">
              ...
              <div>
                <h4 className="font-medium text-amber-800 text-xs sm:text-sm">Título profesional</h4>
                <p className="text-[10px] sm:text-xs text-amber-700 mt-0.5">El título profesional se actualiza automáticamente según tu género seleccionado en la información personal.</p>
                <div className="mt-1.5 sm:mt-2 grid grid-cols-1 md:grid-cols-3 gap-1.5 sm:gap-2 text-[10px] sm:text-xs">
                  <div className="bg-white/70 backdrop-blur-sm rounded-lg p-1.5 sm:p-2 border border-amber-100">
      ...
      <AnimatePresence>
        {isEditing && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 20 }}
            transition={{ duration: 0.3 }}
            className="flex justify-end space-x-2 sm:space-x-4 pt-3 sm:pt-6"
          >
            <motion.button
              ...
              className="inline-flex justify-center py-1 sm:py-2 px-2 sm:px-4 border border-gray-300 shadow-sm text-xs sm:text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2A6877] transition-all"
            >
              Cancelar
            </motion.button>
            <motion.button
              ...
              type="submit"
              disabled={isSaving}
              className="inline-flex justify-center py-1 sm:py-2 px-3 sm:px-5 border border-transparent shadow-sm text-xs sm:text-sm font-medium rounded-md text-white bg-gradient-to-r from-[#2A6877] to-[#235A67] hover:from-[#235A67] hover:to-[#1A4652] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2A6877] disabled:opacity-50 transition-all"
            >
              {isSaving ? (
                <span className="flex items-center">
                  <svg className="animate-spin -ml-0.5 mr-1.5 sm:mr-2 h-2.5 w-2.5 sm:h-4 sm:w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <span className="flex items-center">
                  Guardar cambios
                  <CheckIcon className="ml-1.5 sm:ml-2 h-2.5 w-2.5 sm:h-4 sm:w-4" />
                </span>
              )}
            </motion.button>
          </motion.div>
