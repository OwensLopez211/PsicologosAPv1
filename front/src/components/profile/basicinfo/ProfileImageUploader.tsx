...
const ProfileImageUploader = ({ 
  ...
  return (
    <motion.div 
      className="flex flex-col md:flex-row md:items-center gap-4 sm:gap-6 p-3 sm:p-6 bg-white rounded-xl shadow-sm"
      animate={{ 
        opacity: 1,
        scale: 1
      }}
    ...
    >
      <div className="relative flex-shrink-0 mx-auto md:mx-0">
        ...
        >
          <div className={`h-24 w-24 sm:h-32 sm:w-32 md:h-36 md:w-36 rounded-full overflow-hidden border-4 ${isEditing ? 'border-[#2A6877]' : 'border-[#B4E4D3]'} shadow-md transition-all duration-300`}>
            <img
              src={getImageSource()}
              alt="Profile"
          ...
          <AnimatePresence>
            {isEditing && isHovered && (
              <motion.div 
                ...
                transition={{ duration: 0.2 }}
              >
                <span className="text-white text-xs sm:text-sm font-medium px-2 sm:px-3 py-1 sm:py-1.5 bg-black/40 rounded-full backdrop-blur-sm">
                  Cambiar foto
                </span>
              </motion.div>
            )}
        ...
        <AnimatePresence>
          {isEditing && (
            <motion.label 
              ...
              className={`absolute bottom-0 right-0 bg-[#2A6877] text-white p-2 sm:p-2.5 rounded-full cursor-pointer hover:bg-[#235A67] transition-all duration-200 shadow-md ${isUploadingImage ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              <input
                type="file"
                className="hidden"
        ...
        <AnimatePresence>
          {isEditing && (hasProfileImage || imagePreview) && (
            <motion.button
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.8 }}
              className="absolute bottom-0 left-0 bg-red-500 text-white p-2 sm:p-2.5 rounded-full cursor-pointer hover:bg-red-600 transition-all duration-200 shadow-md"
              disabled={isUploadingImage}
      ...
      <div className="flex-1 text-center md:text-left mt-2 md:mt-0">
        <h3 className="text-md sm:text-lg font-medium text-gray-900">Foto de Perfil</h3>
        <p className="text-xs sm:text-sm text-gray-600 mb-2 sm:mb-3">Una buena foto profesional aumenta la confianza de tus pacientes</p>
        ...
        <AnimatePresence>
          {uploadError && (
            <motion.div
              ...
              className="text-xs sm:text-sm text-red-600 mb-2 bg-red-50 px-2 sm:px-3 py-1.5 sm:py-2 rounded-md"
            >
              {uploadError}
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {isEditing && (
            <motion.div
              ...
              className="text-xs text-gray-500 bg-gray-50 p-2 sm:p-3 rounded-md border border-gray-100"
            >
              <h4 className="font-medium text-gray-700 mb-1">Recomendaciones:</h4>
              <ul className="list-disc pl-4 space-y-0.5 sm:space-y-1 text-xs">
