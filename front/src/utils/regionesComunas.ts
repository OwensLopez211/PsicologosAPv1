...
// Interfaces útiles para trabajar con los datos
export interface RegionData {
  region: string;
  comunas: string[];
}

export interface RegionesComunasData {
  regiones: RegionData[];
}

// Funciones de utilidad para trabajar con las regiones y comunas

/**
 * Obtiene todas las regiones disponibles
 */
export const getRegiones = (): string[] => {
  return regionesComunas.regiones.map(r => r.region);
};
...
export const getComunasByRegion = (region: string): string[] => {
  const regionData = regionesComunas.regiones.find(r => r.region === region);
  return regionData ? regionData.comunas : [];
};

/**
 * Verifica si una comuna pertenece a una región específica
 * @param region Nombre de la región
 * @param comuna Nombre de la comuna
 */
export const comunaPerteneceARegion = (region: string, comuna: string): boolean => {
